...
class PostCard extends Component {
  ...
  showDelete = () => {
    ...
    const duration = moment
      .duration(now.diff(this.props.post.date))
      .asSeconds();
    console.log("DURATION", duration);
    if (
      duration < this.props.user.duration &&
      this.props.user.myAccount === this.props.post.user
    )
      return (
        <Button
          variant="secondary"
  ...
  render() {
    ...
    return (
      <Card style={{marginBottom: "15px"}}>
        ...
        {post.img ? <Card.Img variant="bottom" src={post.img} /> : null}
        <Card.Footer className="text-muted" style={{ height: "45px" }}>
          {time}
          <p className="text-primary" style={{ float: "right" }}>
            <span onClick={this.likePost} style={{ cursor: "pointer" }}>
              Like
            </span>
            <span
